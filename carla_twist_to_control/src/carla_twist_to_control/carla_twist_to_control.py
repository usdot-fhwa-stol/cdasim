...
class TwistToVehicleControl(object):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, role_name):
        ...
        rospy.Subscriber("vehicle_cmd", VehicleCmd, self.twist_received)

        self.pub = rospy.Publisher("/carla/{}/vehicle_control_cmd".format(role_name),
                                   CarlaEgoVehicleControl, queue_size=1)

    def twist_received(self, vehicle_cmd):
        """
        receive twist and convert to carla vehicle control
        """
        control = CarlaEgoVehicleControl()

        if vehicle_cmd == VehicleCmd():
            # stop
            control.throttle = 0.
            control.brake = 1.
            control.steer = 0.
        else:
            twist = vehicle_cmd.twist_cmd.twist
            rospy.loginfo("twist.linear.x: {}".format(twist.linear.x))
            if twist.linear.x > 0:
                control.throttle = 2*min(TwistToVehicleControl.MAX_LON_ACCELERATION,
