...
# Add CARLA Python API to path (same logic as v0.9 file)
try:
    sys.path.append(
...
def _parse_carla_version() -> Tuple[int, int]:
    try:
        v = getattr(carla, "__version__", None)
        if not v:
            return (0, 0)
        parts = str(v).split('.')
...
class CarlaXMLRPCServer:
    def __init__(self, host: str = 'localhost', port: int = 8090,
        ...
        # Version flags
        maj, minr = _parse_carla_version()
        self.carla_version: Tuple[int, int] = (maj, minr)
        self.is_v010_or_newer: bool = (maj > 0) or (maj == 0 and minr >= 10)

        # External-to-CARLA coordinate transform settings (SUMO/MOSAIC → CARLA)
        ...
        logger.info("XML-RPC methods registered (CARLA %s.%s)", maj, minr)
        ...
        try:
            with self.lock:
                ...
                try:
                    ...
                    current_map = "<unknown>"
                logger.info("Connected to CARLA %s.%s at %s:%s | map=%s",
                            self.carla_version[0], self.carla_version[1], self.carla_host, self.carla_port, current_map)
    ...
    def spawn_actor(self, actor_type: str, actor_id: str,
        ...
        try:
            logger.info("[XMLRPC unified] spawn_actor: type=%s id=%s loc=%s rot=%s attrs=%s", actor_type, actor_id, location, rotation, list((attributes or {}).keys()))
            with self.lock:
                if not self.is_connected():
                    return False
        ...
        try:
            with self.lock:
                ...
                if hasattr(data, 'raw_data') and hasattr(data, 'width') and hasattr(data, 'height'):
                    ...
                    try:
                        meta.setdefault('fov', float(sensor.attributes.get('fov')))
                    except Exception:
                        pass
    ...
    def get_available_maps(self) -> List[str]:
        try:
            ...
            try:
                ...
                maps = []
            if self.is_v010_or_newer and not maps:
...
def main():
    parser = argparse.ArgumentParser(description='CARLA XML-RPC Server (Unified by Version)')
    parser.add_argument('--host', default='localhost')
    parser.add_argument('--port', type=int, default=8090)
    parser.add_argument('--carla-host', default='localhost')
    parser.add_argument('--carla-port', type=int, default=2000)
    parser.add_argument('--tls-manager', type=str, choices=['none', 'sumo', 'carla', 'EVC'], default='none')
