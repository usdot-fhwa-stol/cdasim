...
public class WebsocketVisualizerServerTest {
        ...
        /**
         * Tests if the vehicle updates are reduced to only necessary data before
         * sending them to the client.
         */
        @Test
        public void collectRemovedVehiclesBeforeTransport() {
                // setup
                websocketVisualizer.updateVehicleUpdates(new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(), Lists.newArrayList("veh_0")));
                websocketVisualizer.updateVehicleUpdates(new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(), Lists.newArrayList()));
                websocketVisualizer.updateVehicleUpdates(new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(), Lists.newArrayList("veh_1", "veh_2")));
                ...
                // send another vehicle to be removed -> only this vehicle is expected in the
                // message to be sent
                websocketVisualizer.updateVehicleUpdates(new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(), Lists.newArrayList("veh_4")));
                websocketVisualizer.onMessage(socketMock, (String) null);
        ...
        public void reduceVehicleDataBeforeTransport() {
                ...
                final VehicleData vehicleDataToSend = new VehicleData.Builder(111, "1")
                                .position(GeoPoint.lonLat(11, 10), null).movement(10, 0d, 0d)
                                .orientation(DriveDirection.UNAVAILABLE, -18.0, 0d)
                                .road(new SimpleRoadPosition("prev", "upcoming", 0, 1d)).sensors(vehSensors).create();
                final VehicleUpdates vehMovementsToSend = new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(vehicleDataToSend), Lists.newArrayList());
                ...
                final VehicleUpdates actualVehMovementsReceived = gson.fromJson(
                                jsonElement.getAsJsonObject().get(VehicleUpdates.TYPE_ID), VehicleUpdates.class);

                // assert
                final VehicleData vehicleDataToReceive = new VehicleData.Builder(111, "1")
                                .position(GeoPoint.lonLat(11, 10), null).create();
                final VehicleUpdates vehMovementsToReceive = new VehicleUpdates(0, Lists.newArrayList(),
                                Lists.newArrayList(vehicleDataToReceive), Lists.newArrayList());
