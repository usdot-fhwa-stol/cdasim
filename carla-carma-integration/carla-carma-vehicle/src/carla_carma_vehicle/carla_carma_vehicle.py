...
class CarlaVirtualVehicle(object):

    """
    Handles the spawning of the virtual vehicle and its sensors
    Derive from this class and implement method sensors()
    """

    def __init__(self):
        rospy.init_node('virtual_vehicle', anonymous=True)
    ...
    def restart(self):
        ...
        # Spawn the vehicle.
        if not rospy.get_param('~spawn_virtual_vehicle'):
    ...
    def setup_sensors(self, sensors):
        """
        Create the sensors defined by the user and attach them to the virtual-vehicle
    ...
    def destroy(self):
        """
        destroy the current virtual vehicle and its sensors
        """
    ...
    def run(self):
        ...
        rospy.loginfo("CARLA world available. Spawn carma-config virtual vehicle...")
...
def main():
    ...
    main function
    """
    virtual_vehicle = CarlaVirtualVehicle()
    try:
        virtual_vehicle.run()
    finally:
        if virtual_vehicle is not None:
            virtual_vehicle.destroy()
